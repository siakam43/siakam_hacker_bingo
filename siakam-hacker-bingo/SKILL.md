...
**Retry rule:** Unless the check returns a definitive "file exists" result, you MUST try a second time with a different method (different command or different tool):
...
**If the check confirms the file exists:** Read `task-list.md`. Skip all tasks with status `done`. Find the first task with status `pending` or `in_progress`. Jump to Step 3 (Dispatch Loop) starting from that task. Tasks marked `in_progress` were interrupted — re-run them.
